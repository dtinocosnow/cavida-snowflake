...
for ft in ['BOND','EQUITY','CASH']:
    with st.expander(f"📁 REGISTOS VAZIOS — {ft}"):
        d=session.sql(f"SELECT risk_agility_file AS \"Ficheiro\",column_name AS \"Coluna\",information AS \"Info\" FROM CAVIDA_DEMO.BRONZE.SLV2_RISK_AGILITY_INPUTS_ASSETS WHERE reference_date='2025-06-30' AND risk_agility_file='{ft}'").to_pandas()
