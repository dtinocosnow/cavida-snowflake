...
with col2:
    ...
    st.dataframe(session.sql("SELECT reference_date,COUNT(*)as registos,MAX(loaded_at)as ultima FROM CAVIDA_DEMO.BRONZE.SLV2_INVESTMENT_PORTFOLIO GROUP BY 1 ORDER BY 1 DESC LIMIT 5").to_pandas(), use_container_width=True, hide_index=True)
