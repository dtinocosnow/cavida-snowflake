...
if st.button("🔒 Fecho Report SLV II",type="primary",key="close15"):
    ...
    session.sql("UPDATE CAVIDA_DEMO.GOLD.SLV2_REPORT_STATUS SET status='Fechado',closed_by=CURRENT_USER(),closed_at=CURRENT_TIMESTAMP() WHERE reference_date='2025-06-30' AND status='Em Curso'").collect()
...
df=session.sql("SELECT report_name AS \"Relatório\",reference_date AS \"Data\",status AS \"Status\",closed_by AS \"Fechado por\",closed_at AS \"Data Fecho\" FROM CAVIDA_DEMO.GOLD.SLV2_REPORT_STATUS ORDER BY reference_date DESC").to_pandas()
def cs(v):
